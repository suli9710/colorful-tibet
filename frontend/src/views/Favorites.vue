  ...
  <div class="min-h-screen bg-stone-50 py-12 pt-24">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-stone-800 mb-4">{{ $t('favorites.title') || '我的收藏' }}</h1>
        <p class="text-lg text-stone-600">{{ $t('favorites.subtitle') || '已收藏的旅游线路' }}</p>
      </div>
      ...
      <div v-else-if="favorites.length === 0" class="text-center py-16">
        <p class="text-stone-500 text-lg">{{ $t('favorites.empty') || '还没有收藏任何线路' }}</p>
      ...
      <div v-else class="space-y-4">
        <div v-for="fav in favorites" :key="fav.id" class="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow">
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h3 class="text-lg font-bold text-stone-800">{{ fav.route?.name || '未知线路' }}</h3>
            ...
            <button @click="removeFavorite(fav.route?.id)" class="text-red-500 hover:text-red-700" :title="$t('favorites.remove') || '取消收藏'">
