# 腾讯云部署详细教程

本文档提供在腾讯云服务器上部署"多彩西藏"旅游网站的完整详细教程，适合从零开始的用户。

## 📋 目录

1. [准备工作](#准备工作)
2. [服务器环境准备](#服务器环境准备)
3. [项目代码部署](#项目代码部署)
4. [Docker 环境安装](#docker-环境安装)
5. [环境变量配置](#环境变量配置)
6. [服务启动和验证](#服务启动和验证)
7. [域名和 SSL 配置（可选）](#域名和-ssl-配置可选)
8. [常见问题排查](#常见问题排查)
9. [日常维护和更新](#日常维护和更新)

---

## 准备工作

### 1.1 购买腾讯云服务器

#### 服务器规格推荐

- **最低配置**：
  - CPU: 2核
  - 内存: 4GB
  - 硬盘: 50GB SSD
  - 带宽: 3Mbps

- **推荐配置**（生产环境）：
  - CPU: 4核
  - 内存: 8GB
  - 硬盘: 100GB SSD
  - 带宽: 5Mbps

#### 操作系统选择

推荐使用以下操作系统之一：
- **Ubuntu 22.04 LTS**（推荐）
- **Ubuntu 20.04 LTS**
- **CentOS 7.9**

本教程以 **Ubuntu 22.04** 为例。

#### 购买步骤

1. 登录腾讯云控制台
2. 进入「云服务器 CVM」页面
3. 点击「新建」创建实例
4. 选择配置：
   - 地域：选择离用户最近的地域
   - 机型：标准型 S5
   - 镜像：Ubuntu Server 22.04 LTS 64位
   - 系统盘：50GB SSD云硬盘
   - 网络：默认网络
   - 公网IP：分配公网IP
   - 带宽：按量计费，3Mbps
5. 设置登录方式：
   - 推荐使用「SSH密钥对」或「立即设置密码」
6. 配置安全组（见下一节）
7. 完成购买

### 1.2 配置安全组规则

安全组是云服务器的防火墙，需要开放以下端口：

#### 必需端口

| 端口 | 协议 | 说明 | 来源 |
|------|------|------|------|
| 22 | TCP | SSH远程登录 | 0.0.0.0/0（建议限制为您的IP） |
| 80 | TCP | HTTP访问 | 0.0.0.0/0 |
| 443 | TCP | HTTPS访问 | 0.0.0.0/0 |
| 8080 | TCP | 后端API（可选，如果不需要外部直接访问可关闭） | 0.0.0.0/0 |

#### 配置步骤

1. 在云服务器控制台，找到您的实例
2. 点击「安全组」标签
3. 点击「修改规则」
4. 点击「添加规则」，依次添加上述端口规则
5. 保存规则

**安全建议**：
- SSH端口（22）建议只允许您的IP访问
- 生产环境建议关闭8080端口的外部访问，只允许通过80端口访问

### 1.3 准备必需信息

部署前需要准备以下信息：

- ✅ **服务器IP地址**：在云服务器控制台查看
- ✅ **SSH登录凭证**：密码或SSH密钥
- ✅ **豆包API Key**：用于AI推荐功能
- ✅ **豆包API URL**：API接口地址
- ✅ **豆包模型名称**：使用的模型名称
- ✅ **域名**（可选）：如果使用域名访问
- ✅ **Git仓库地址**（如果使用Git部署）

---

## 服务器环境准备

### 2.1 连接到服务器

#### Windows 用户

**方法一：使用 PowerShell**

```powershell
# 使用密码登录
ssh root@您的服务器IP

# 或使用密钥文件
ssh -i C:\path\to\your\key.pem root@您的服务器IP
```

**方法二：使用 PuTTY**

1. 下载并安装 PuTTY
2. 输入服务器IP地址
3. 选择连接类型：SSH
4. 点击「打开」
5. 输入用户名（root）和密码

#### Mac/Linux 用户

```bash
# 使用密码登录
ssh root@您的服务器IP

# 或使用密钥文件
ssh -i ~/.ssh/your_key.pem root@您的服务器IP
```

#### ⚠️ SSH 连接常见问题

**问题1：主机密钥已更改警告**

如果看到 `WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!` 错误，这通常发生在：
- 服务器重装系统后
- 服务器更换了SSH密钥
- 服务器IP被重新分配

**解决方法（Windows PowerShell）：**

```powershell
# 方法1：删除known_hosts文件中的旧密钥（推荐）
ssh-keygen -R 1.15.29.168

# 方法2：手动编辑known_hosts文件，删除第3行（根据错误提示）
notepad C:\Users\Suli\.ssh\known_hosts
# 删除包含该IP地址的行，保存后重试

# 方法3：临时跳过主机密钥检查（不推荐，仅用于测试）
ssh -o StrictHostKeyChecking=no root@1.15.29.168
```

**解决方法（Mac/Linux）：**

```bash
# 删除旧密钥
ssh-keygen -R 1.15.29.168

# 或手动编辑
nano ~/.ssh/known_hosts
# 删除包含该IP的行，保存后重试
```

删除旧密钥后，重新连接时会提示接受新的主机密钥，输入 `yes` 即可。

**问题2：连接超时**

```powershell
# 检查网络连通性
ping 1.15.29.168

# 检查端口是否开放
Test-NetConnection -ComputerName 1.15.29.168 -Port 22
```

**问题3：权限被拒绝（Permission denied）**

如果看到 `Permission denied, please try again.` 错误，请按以下步骤排查：

**步骤1：确认用户名**

不同操作系统的默认用户名可能不同：
- **Ubuntu**: 通常是 `ubuntu`（不是 `root`）
- **CentOS**: 通常是 `root`
- **Debian**: 通常是 `root`

尝试不同的用户名：
```powershell
# 尝试 ubuntu 用户（Ubuntu系统常见）
ssh ubuntu@1.15.29.168

# 或尝试 root 用户
ssh root@1.15.29.168
```

**步骤2：确认密码**

- 密码输入时**不会显示任何字符**（包括星号），这是正常的安全机制
- 确认密码是否正确（注意大小写、特殊字符）
- 如果是在腾讯云购买的服务器，密码通常在：
  - 购买时设置的密码
  - 控制台「实例」→「重置密码」中设置的密码

**步骤3：重置服务器密码**

如果忘记密码，可以在腾讯云控制台重置：

1. 登录腾讯云控制台
2. 进入「云服务器 CVM」→「实例」
3. 找到您的服务器，点击「更多」→「密码/密钥」→「重置密码」
4. 设置新密码（需要重启服务器生效）
5. 重启服务器后，使用新密码登录

**步骤4：检查是否使用SSH密钥**

如果服务器配置了SSH密钥登录，可能需要使用密钥文件：

```powershell
# 使用密钥文件登录
ssh -i C:\path\to\your\key.pem ubuntu@1.15.29.168

# 或 root 用户
ssh -i C:\path\to\your\key.pem root@1.15.29.168
```

**步骤5：使用腾讯云控制台登录（临时方案）**

如果SSH无法连接，可以：
1. 在腾讯云控制台找到您的服务器
2. 点击「登录」按钮
3. 使用浏览器控制台登录（VNC方式）
4. 登录后可以重置密码或配置SSH密钥

**步骤6：检查安全组设置**

确认安全组已开放22端口：
- 在腾讯云控制台检查安全组规则
- 确保22端口对您的IP开放（或0.0.0.0/0开放）

**常见错误示例：**

```powershell
# ❌ 错误：用户名可能不对
ssh root@1.15.29.168  # 如果系统默认用户是 ubuntu，这会失败

# ✅ 正确：尝试 ubuntu 用户
ssh ubuntu@1.15.29.168

# ✅ 如果设置了密钥，使用密钥登录
ssh -i your-key.pem ubuntu@1.15.29.168
```

### 2.2 更新系统

首次登录后，先更新系统：

```bash
# 更新软件包列表
apt update

# 升级已安装的软件包
apt upgrade -y

# 安装常用工具
apt install -y curl wget git vim net-tools
```

### 2.3 创建非root用户（推荐，可选）

为了安全，建议创建普通用户：

```bash
# 创建用户（替换 your_username 为您的用户名）
adduser your_username

# 将用户添加到sudo组
usermod -aG sudo your_username

# 切换到新用户
su - your_username
```

---

## 项目代码部署

### 3.1 方法一：使用 Git 克隆（推荐）

#### 步骤 1：创建项目目录

```bash
# 创建项目目录
sudo mkdir -p /opt/colorful-tibet
sudo chown -R $USER:$USER /opt/colorful-tibet
cd /opt/colorful-tibet
```

#### 步骤 2：克隆代码

```bash
# 公开仓库
git clone https://github.com/suli9710/colorful-tibet.git .

# 或私有仓库（需要配置SSH密钥或使用HTTPS认证）
git clone git@github.com:suli9710/colorful-tibet.git .
```

#### 步骤 3：检查代码

```bash
# 查看项目结构
ls -la

# 应该看到以下目录：
# backend/  frontend/  docker-compose.yml  deploy-tencent-cloud.sh 等
```

### 3.2 方法二：使用 SCP 上传

如果您在本地有项目代码，可以使用 SCP 上传：

#### Windows PowerShell

```powershell
# 上传整个项目目录
scp -r C:\Users\Suli\Desktop\colorful-tibet root@您的服务器IP:/opt/
```

#### Mac/Linux

```bash
# 上传整个项目目录
scp -r ~/Desktop/colorful-tibet root@您的服务器IP:/opt/
```

#### 在服务器上验证

```bash
# 进入项目目录
cd /opt/colorful-tibet

# 检查文件
ls -la
```

### 3.3 设置文件权限

```bash
cd /opt/colorful-tibet

# 给部署脚本添加执行权限
chmod +x deploy-tencent-cloud.sh
chmod +x deploy.sh

# 确保当前用户拥有项目目录
sudo chown -R $USER:$USER /opt/colorful-tibet
```

---

## Docker 环境安装

### 4.1 安装 Docker

#### 方法一：使用官方安装脚本（推荐）

```bash
# 下载并运行Docker安装脚本
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# 启动Docker服务
sudo systemctl start docker
sudo systemctl enable docker

# 验证安装
docker --version
```

#### 方法二：使用包管理器安装

```bash
# 更新软件包索引
sudo apt update

# 安装必要的依赖
sudo apt install -y ca-certificates curl gnupg lsb-release

# 添加Docker官方GPG密钥
sudo mkdir -p /etc/apt/keyrings
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg

# 设置仓库
echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \
  $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

# 安装Docker Engine
sudo apt update
sudo apt install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin

# 启动Docker服务
sudo systemctl start docker
sudo systemctl enable docker

# 验证安装
docker --version
```

### 4.2 配置 Docker（可选）

#### 将用户添加到docker组

这样就不需要每次都用sudo：

```bash
# 将当前用户添加到docker组
sudo usermod -aG docker $USER

# 重新登录或执行以下命令使配置生效
newgrp docker

# 验证（不需要sudo）
docker ps
```

#### 配置Docker镜像加速（可选，推荐）

如果下载镜像较慢，可以配置国内镜像源：

```bash
# 创建或编辑daemon.json
sudo mkdir -p /etc/docker
sudo tee /etc/docker/daemon.json <<-'EOF'
{
  "registry-mirrors": [
    "https://docker.mirrors.ustc.edu.cn",
    "https://hub-mirror.c.163.com",
    "https://mirror.baidubce.com"
  ]
}
EOF

# 重启Docker服务
sudo systemctl daemon-reload
sudo systemctl restart docker

# 验证配置
docker info | grep -A 10 "Registry Mirrors"
```

### 4.3 安装 Docker Compose

Docker Compose V2 通常随 Docker 一起安装。验证是否已安装：

```bash
# 检查Docker Compose版本
docker compose version
```

如果未安装，可以手动安装：

```bash
# 安装Docker Compose插件
sudo apt install -y docker-compose-plugin

# 验证安装
docker compose version
```

---

## 环境变量配置

### 5.1 创建 .env 文件

```bash
cd /opt/colorful-tibet

# 创建.env文件
nano .env
```

### 5.2 配置必需的环境变量

在编辑器中输入以下内容（**请替换为您的实际值**）：

```env
# ============================================
# 豆包 API 配置（必需）
# ============================================
DOUBAO_API_KEY=your-doubao-api-key-here
DOUBAO_API_URL=https://ark.cn-beijing.volces.com/api/v3/chat/completions
DOUBAO_MODEL=your-model-name-here

# ============================================
# JWT 配置（必需）
# ============================================
# 生成方法: openssl rand -hex 32
JWT_SECRET=your-generated-jwt-secret-key-here

# ============================================
# 管理员加密密钥（必需）
# ============================================
# 生成方法: openssl rand -hex 32
ADMIN_ENCRYPTION_KEY=your-generated-admin-encryption-key-here

# ============================================
# 前端 API 地址（必需）
# ============================================
# 替换为您的服务器IP或域名
VITE_API_BASE_URL=http://您的服务器IP:8080/api

# ============================================
# Spring Profile（可选）
# ============================================
# local: 使用H2内存数据库（开发/测试）
# prod: 使用MySQL数据库（生产环境）
SPRING_PROFILES_ACTIVE=local
```

### 5.3 生成随机密钥

使用以下命令生成安全的随机密钥：

```bash
# 生成JWT密钥
openssl rand -hex 32

# 生成管理员加密密钥
openssl rand -hex 32
```ebf7c93d46659b65a154586cc8fddf1949643bd8b42f4377cfc1643da64460bc
ubuntu@VM-16-10-ubuntu:/opt/colorful-tibet$ openssl rand -hex 32
cfbe12cb6d3ca9fd8f37114d57cbf6c03eebfff9f81d98e221e1aabb89457710

将生成的密钥复制到 `.env` 文件中。

### 5.4 获取服务器IP地址

```bash
# 方法1：查看公网IP
curl ifconfig.me

# 方法2：查看内网IP
hostname -I

# 方法3：在腾讯云控制台查看
```

将公网IP填入 `VITE_API_BASE_URL`。

### 5.5 保存并验证配置

```bash
# 保存文件（在nano中：Ctrl+O保存，Ctrl+X退出）

# 验证文件内容
cat .env

# 检查文件权限（确保只有所有者可读）
chmod 600 .env
ls -la .env
```

---

## 服务启动和验证

### 6.1 使用一键部署脚本（推荐）

项目提供了自动部署脚本，可以自动完成大部分配置：

```bash
cd /opt/colorful-tibet

# 如果使用root用户
./deploy-tencent-cloud.sh

# 如果使用普通用户，需要sudo
sudo ./deploy-tencent-cloud.sh
```

脚本会自动：
- ✅ 检查并安装Docker
- ✅ 检查Docker Compose
- ✅ 配置防火墙规则
- ✅ 创建必要的目录
- ✅ 生成环境变量文件（如果不存在）
- ✅ 更新docker-compose.yml配置
- ✅ 构建Docker镜像
- ✅ 启动服务

### 6.2 手动启动服务

如果自动脚本无法使用，可以手动启动：

#### 步骤 1：更新 docker-compose.yml（如果需要）

编辑 `docker-compose.yml`，确保 `VITE_API_BASE_URL` 正确：

```bash
nano docker-compose.yml
```

找到 `frontend` 服务的 `args` 部分，确保：

```yaml
args:
  - VITE_API_BASE_URL=${VITE_API_BASE_URL:-/api}
```

#### 步骤 2：构建镜像

```bash
cd /opt/colorful-tibet

# 构建所有服务镜像（首次构建可能需要10-20分钟）
docker compose build

# 或只构建特定服务
docker compose build backend
docker compose build frontend
```

#### 步骤 3：启动服务

```bash
# 启动所有服务（后台运行）
docker compose up -d

# 查看启动日志
docker compose logs -f
```

#### 步骤 4：检查服务状态

```bash
# 查看容器状态
docker compose ps

# 应该看到两个容器都在运行：
# colorful-tibet-backend   Up
# colorful-tibet-frontend  Up
```

### 6.3 验证服务

#### 检查后端服务

```bash
# 方法1：检查容器日志
docker compose logs backend

# 方法2：测试API接口
curl http://localhost:8080/api/spots

# 方法3：检查健康状态
curl http://localhost:8080/api/health
```

如果看到JSON数据返回，说明后端正常运行。

#### 检查前端服务

```bash
# 方法1：检查容器日志
docker compose logs frontend

# 方法2：测试HTTP响应
curl http://localhost

# 应该返回HTML内容
```

#### 检查外部访问

在浏览器中访问：
- **前端**: http://您的服务器IP
- **后端API**: http://您的服务器IP:8080/api/spots

如果能看到页面或数据，说明部署成功！

### 6.4 查看日志

```bash
# 查看所有服务日志
docker compose logs -f

# 查看后端日志
docker compose logs -f backend

# 查看前端日志
docker compose logs -f frontend

# 查看最近100行日志
docker compose logs --tail=100 backend
```

---

## 域名和 SSL 配置（可选）

### 7.1 配置域名解析

1. 登录您的域名服务商（如腾讯云、阿里云等）
2. 添加A记录：
   - 主机记录：`@` 或 `www`
   - 记录类型：A
   - 记录值：您的服务器IP地址
   - TTL：600（10分钟）

3. 等待DNS解析生效（通常几分钟到几小时）

### 7.2 安装 Certbot（Let's Encrypt）

```bash
# 安装Certbot
sudo apt update
sudo apt install -y certbot python3-certbot-nginx

# 验证安装
certbot --version
```

### 7.3 配置 Nginx（如果需要）

如果使用域名，需要配置Nginx反向代理。首先安装Nginx：

```bash
sudo apt install -y nginx
```

创建Nginx配置文件：

```bash
sudo nano /etc/nginx/sites-available/colorful-tibet
```

添加以下配置（替换 `your-domain.com` 为您的域名）：

```nginx
server {
    listen 80;
    server_name your-domain.com www.your-domain.com;

    # 前端
    location / {
        proxy_pass http://localhost:80;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }

    # 后端API
    location /api {
        proxy_pass http://localhost:8080;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }
}
```

启用配置：

```bash
# 创建符号链接
sudo ln -s /etc/nginx/sites-available/colorful-tibet /etc/nginx/sites-enabled/

# 测试配置
sudo nginx -t

# 重载Nginx
sudo systemctl reload nginx
```

### 7.4 申请 SSL 证书

```bash
# 申请证书（替换为您的域名和邮箱）
sudo certbot --nginx -d your-domain.com -d www.your-domain.com

# 按照提示操作：
# 1. 输入邮箱地址
# 2. 同意服务条款
# 3. 选择是否接收邮件通知
# 4. Certbot会自动配置SSL
```

### 7.5 更新环境变量

申请SSL证书后，更新 `.env` 文件：

```bash
nano /opt/colorful-tibet/.env
```

修改 `VITE_API_BASE_URL`：

```env
VITE_API_BASE_URL=https://your-domain.com/api
```

重新构建前端镜像：

```bash
cd /opt/colorful-tibet
docker compose build frontend
docker compose up -d frontend
```

### 7.6 自动续期证书

Let's Encrypt证书有效期90天，设置自动续期：

```bash
# 测试续期
sudo certbot renew --dry-run

# 证书会自动续期（certbot已配置定时任务）
```

---

## 常见问题排查

### 8.1 服务无法启动

#### 问题：容器启动后立即退出

```bash
# 查看详细日志
docker compose logs backend
docker compose logs frontend

# 检查容器状态
docker compose ps -a

# 查看容器退出代码
docker inspect colorful-tibet-backend | grep -A 10 "State"
```

**常见原因**：
- 环境变量配置错误
- 端口被占用
- 镜像构建失败

#### 解决方案

```bash
# 检查端口占用
sudo netstat -tlnp | grep 8080
sudo netstat -tlnp | grep 80

# 如果端口被占用，停止占用进程或修改docker-compose.yml中的端口映射

# 重新构建镜像
docker compose build --no-cache

# 重新启动
docker compose up -d
```

### 8.2 前端无法访问后端

#### 问题：前端页面显示API错误

**检查步骤**：

1. **检查后端是否运行**：
```bash
curl http://localhost:8080/api/spots
```

2. **检查环境变量配置**：
```bash
cat .env | grep VITE_API_BASE_URL
```

3. **检查前端构建时的API地址**：
```bash
docker compose config | grep VITE_API_BASE_URL
```

**解决方案**：

如果 `VITE_API_BASE_URL` 配置错误，需要重新构建前端：

```bash
# 修改.env文件
nano .env

# 重新构建前端镜像
docker compose build frontend

# 重启前端容器
docker compose up -d frontend
```

### 8.3 数据库连接失败

#### 问题：后端日志显示数据库连接错误

**如果使用H2数据库（local profile）**：

检查 `application-local.yml` 配置是否正确。

**如果使用MySQL（prod profile）**：

1. **检查MySQL服务**：
```bash
# 如果MySQL在Docker容器中
docker compose ps mysql

# 如果MySQL在宿主机
sudo systemctl status mysql
```

2. **检查数据库配置**：
```bash
cat .env | grep DB_
```

3. **测试数据库连接**：
```bash
mysql -h localhost -u tibet_user -p tibet_tourism
```

### 8.4 内存不足

#### 问题：服务运行缓慢或崩溃

```bash
# 查看内存使用
free -h

# 查看Docker资源使用
docker stats

# 查看系统资源
htop  # 需要安装: apt install htop
```

**解决方案**：

1. 增加服务器内存
2. 优化Docker资源限制
3. 关闭不必要的服务

### 8.5 防火墙问题

#### 问题：外部无法访问服务

```bash
# 检查UFW防火墙状态
sudo ufw status

# 检查iptables规则
sudo iptables -L -n

# 检查腾讯云安全组配置（在控制台检查）
```

**解决方案**：

```bash
# 开放端口
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp
sudo ufw allow 8080/tcp

# 启用防火墙
sudo ufw enable
```

### 8.6 Docker镜像下载慢

**解决方案**：配置镜像加速（见4.2节）

### 8.7 权限问题

#### 问题：Permission denied

```bash
# 检查文件权限
ls -la /opt/colorful-tibet

# 修改所有者
sudo chown -R $USER:$USER /opt/colorful-tibet

# 检查Docker权限
docker ps  # 如果失败，需要将用户添加到docker组
```

---

## 日常维护和更新

### 9.1 更新代码

#### 方法一：使用Git（推荐）

```bash
cd /opt/colorful-tibet

# 拉取最新代码
git pull

# 重新构建镜像
docker compose build

# 重启服务
docker compose up -d
```

#### 方法二：使用部署脚本

```bash
cd /opt/colorful-tibet
./deploy.sh all
```

### 9.2 备份数据

#### 备份数据库

```bash
# 如果使用H2数据库，备份数据文件
cp -r /opt/colorful-tibet/data /opt/backup/data-$(date +%Y%m%d)

# 如果使用MySQL
mysqldump -u tibet_user -p tibet_tourism > /opt/backup/tibet_tourism-$(date +%Y%m%d).sql
```

#### 备份配置文件

```bash
# 备份.env文件
cp /opt/colorful-tibet/.env /opt/backup/.env-$(date +%Y%m%d)
```

### 9.3 查看服务状态

```bash
# 查看容器状态
docker compose ps

# 查看资源使用
docker stats

# 查看日志
docker compose logs --tail=50 -f
```

### 9.4 重启服务

```bash
# 重启所有服务
docker compose restart

# 重启特定服务
docker compose restart backend
docker compose restart frontend

# 完全停止并重新启动
docker compose down
docker compose up -d
```

### 9.5 清理资源

```bash
# 清理未使用的镜像
docker image prune -a

# 清理未使用的容器和网络
docker system prune

# 清理所有未使用的资源（谨慎使用）
docker system prune -a --volumes
```

### 9.6 监控和日志

#### 设置日志轮转

创建日志轮转配置：

```bash
sudo nano /etc/logrotate.d/colorful-tibet
```

添加：

```
/opt/colorful-tibet/logs/*.log {
    daily
    rotate 7
    compress
    delaycompress
    missingok
    notifempty
    create 0644 root root
}
```

#### 监控服务健康

可以设置定时任务检查服务：

```bash
# 编辑crontab
crontab -e

# 添加健康检查（每5分钟检查一次）
*/5 * * * * curl -f http://localhost:8080/api/spots > /dev/null 2>&1 || docker compose restart backend
```

---

## 📞 获取帮助

如果遇到问题：

1. **查看日志**：`docker compose logs`
2. **检查文档**：查看项目中的其他文档
3. **检查GitHub Issues**：在项目仓库中搜索相关问题
4. **联系技术支持**：提供详细的错误日志和配置信息

---

## ✅ 部署检查清单

部署完成后，请确认：

- [ ] 服务器已购买并配置安全组
- [ ] SSH可以正常连接
- [ ] Docker和Docker Compose已安装
- [ ] 项目代码已部署到服务器
- [ ] `.env`文件已配置所有必需的环境变量
- [ ] 服务已成功启动（`docker compose ps`显示Up状态）
- [ ] 前端可以访问（http://服务器IP）
- [ ] 后端API可以访问（http://服务器IP:8080/api/spots）
- [ ] 日志中没有错误信息
- [ ] 防火墙和安全组已正确配置
- [ ] （可选）域名和SSL证书已配置

---

**祝您部署顺利！** 🎉

如有问题，请参考本文档的「常见问题排查」部分，或查看项目中的其他文档。

