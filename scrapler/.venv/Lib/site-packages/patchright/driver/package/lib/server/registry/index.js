...
Run "${buildPlaywrightCLICommand(sdkLanguage, "install " + name)}"` : "";
  ...
  async _downloadExecutable(descriptor, force, executablePath) {
    ...
    if (!downloadURLs.length)
      throw new Error(`ERROR: Patchright does not support ${descriptor.name} on ${import_hostPlatform.hostPlatform}`);
    if (!import_hostPlatform.isOfficiallySupportedPlatform)
      (0, import_browserFetcher.logPolitely)(`BEWARE: your OS is not officially supported by Patchright; downloading fallback build for ${import_hostPlatform.hostPlatform}.`);
    if (descriptor.hasRevisionOverride) {
      ...
      if (process.env.GITHUB_ACTIONS)
        console.log(`::warning title=Patchright::${message}`);
      else
