...
class Frame extends import_instrumentation.SdkObject {
  ...
  async _context(world) {
    ...
    const registerContext = (executionContextId, worldName) => {
      const crContext = new import_crExecutionContext.CRExecutionContext(client, { id: executionContextId }, this._id);
      const frameContext = new import_dom.FrameExecutionContext(crContext, this, worldName);
    ...
    if (world === "main") {
      if (!isMainFrame && iframeExecutionContextId && this._iframeWorld === void 0) {
  ...
  async _retryWithoutProgress(progress, selector, options, action, returnAction, continuePolling) {
    ...
    const resolved = await this.selectors.resolveInjectedForSelector(
      ...
      options.__patchrightInitialScope
    );
    if (!resolved) {
    ...
    if (resolved.scope) {
      const scopeObjectId = resolved.scope._objectId;
      if (scopeObjectId) {
        ...
        for (const scope of currentScopingElements) {
          ...
          const shadowRoots = await Promise.all(
            shadowRootBackendIds.map(async (backendNodeId) => {
              ...
              return new dom.ElementHandle(context, resolved2.object.objectId);
            })
          );
          const queryGroups = [];
          for (var shadowRoot of shadowRoots) {
            const shadowHandles = await shadowRoot.evaluateHandleInUtility(
              ([injected, node, { parsed: parsed2, callId }]) => {
                const elements2 = injected.querySelectorAll(parsed2, node);
                if (callId)
                  injected.markTargetElements(new Set(elements2), callId);
                return elements2;
              },
              {
                parsed: parsedEdits,
